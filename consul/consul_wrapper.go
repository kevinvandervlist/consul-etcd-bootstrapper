...
func (c *ConsulWrapper) Run(bin string, ip string, expectedNodeCnt int, nodes []string) int {
	...
	cli := strings.Join([]string{"agent -server ", unquotedArgs," -bootstrap-expect ", strconv.Itoa(expectedNodeCnt), " -advertise ", ip, " -bind ", ip, " -retry-join ", strings.Join(nodes, " -retry-join ")}, "")
